...
class ProviderFileManager(
    ...
    private val media: MediaContentHelper
) {
    ...
    fun insertImageToStore(fileInfo: FileInfo?) {
        fileInfo?.let {
            insertToStore(
                it,
                media.getImageContentUri(),
                media.generateImageContentValues(it)
            )
        }
    }
    ...
    fun insertVideoToStore(fileInfo: FileInfo?) {
        fileInfo?.let {
            insertToStore(
                it,
                media.getVideoContentUri(),
                media.generateVideoContentValues(it)
            )
        }
    }
